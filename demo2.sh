...
TOKEN_VIDEO=`curl -X GET "${SRV}/auth2/tester/${STREAM_VIDEO}/1" -H "accept: text/plain" 2>/dev/null`
TOKEN_RECORDING=`curl -X GET "${SRV}/auth2/tester/2" -H "accept: text/plain" 2>/dev/null`
TOKEN_RTMP=`curl -X GET "${SRV}/auth2/tester/${STREAM_RTMP}/3" -H "accept: text/plain" 2>/dev/null`
TOKEN_RTMP_HLS=`curl -X GET "${SRV}/auth2/tester/${STREAM_RTMP_HLS}/3" -H "accept: text/plain" 2>/dev/null`
TOKEN_CALL_1=`curl -X GET "${SRV}/auth2/tester/${STREAM_CALL_1}/1" -H "accept: text/plain" 2>/dev/null`
TOKEN_CALL_2=`curl -X GET "${SRV}/auth2/tester/${STREAM_CALL_2}/1" -H "accept: text/plain" 2>/dev/null`
TOKEN_CALL_MIXER=`curl -X GET "${SRV}/auth2/tester/${STREAM_CALL_MIXER}/4" -H "accept: text/plain" 2>/dev/null`
